<template>
  <q-layout view="hHh LpR fFf" :class="[this.$route.name == 'quran_detail' ? 'bg-secondary' : 'mosque-bg']">

    <q-header class="text-white">
      <q-toolbar>
        
        <q-btn v-if="this.$route.name == 'quran_detail' || this.$route.name == 'zikr_detail'" 
          v-go-back.single flat icon="arrow_back" color="white" size="md" />
        <q-avatar v-if="routeName" class="lt-sm">
          <img src="../assets/img/tasbeeh.png" alt="">
        </q-avatar>
        <q-btn dense flat round icon="menu" class="gt-xs" @click="left = !left" />
    ...
    <!--  Desktop Navbar   -->
    <q-drawer show-if-above v-model="left" side="left" class="gt-xs">
      ...
      <q-list class="q-pa-sm ">
        <q-item 
          ...
          v-for="link in links" :key="link.name" :to="link.path" 
...
export default {
  ...
  computed: {
    ...
    routeName() {
      return this.$route.name != 'quran_detail' && this.$route.name != 'about' && this.$route.name != 'zikr_detail'     
    }
  }
}
</script>
