<template>
  <q-page class="">
    ...
    <div class="surah q-py-md text-white q-mx-auto" v-if="surah || !loading" style="max-width: 700px">
      ...
      <p class="text-h6" v-if="surah.preBismillah"> {{surah.preBismillah.text.ar}} </p>
      <p v-for="sura in surah.ayahs" :key="sura.number.inquran" class="text-h6 border-b q-px-md">
        {{sura.text.ar}}<br>
...
export default {
  ...
  methods: {
    getSurahById() {
      this.$axios.get(`https://quran-endpoint.vercel.app/quran/${this.id}`)
        .then((res) => {
